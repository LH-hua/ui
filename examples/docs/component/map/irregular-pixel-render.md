...
<div>
  ...
  <div class="irregular-pixel-render" style="height:626px">
    <s-map
      mapkey="map3"
      ...
      layerDatas: [
        {
          ...
          renderFactor: models[0].renderFactor,
          // 值 对应的颜色
  ...
  methods: {
    regionChange() {
      ...
      this.layerDatas = [{
        ...
        width: model.width,
        height: model.height,
        renderFactor: model.renderFactor
      }]
    },
    clipChange(){
      this.layerDatas = [{
...
| 参数 | 说明 | 类型 | 可选值 | 默认值 | 备注 |
| ---- | ---- | ---- | ---- | ---- | ---- |
| type | 图层类型 | String | irregularPixelRender | — | |
| url | 数据图片地址 | String | — | — | |
| name | 图层名 | String | | — | |
| code | 当前图片对应的指标编码，会在 dataAnalysisFunc 函数中回传 | String | | | |
| renderFactor | 像素渲染间隔系数 | Number | | | 需要根据栅格场调整合适的系数 |
