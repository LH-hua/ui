...
class ADLayer {
  constructor(ecOptions, map, echarts, echartEvents) {
    this._map = map;
    ...
    this.event = null;
    this.echartEvents = echartEvents || []
  ...
  _render() {
    ...
    if (!this._ec) {
      ...
      this.echartEvents.forEach(item => {
        if (item.target) {
          this._ec.on(item.type, item.target, function (params) {
            typeof item.callBack === 'function' && item.callBack(params)
          });
        } else {
          this._ec.on(item.type, function (params) {
            typeof item.callBack === 'function' && item.callBack(params)
          });
        }
      })
  ...
  _getE3CoordinateSystem(map) {
    const CoordSystem = function CoordSystem(map) {
      this.map = map;
      this._mapOffset = [0, 0];
    };
