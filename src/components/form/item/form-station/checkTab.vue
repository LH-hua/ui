<template>
  <div class="list-tab">
    <a-checkable-tag key="all" :checked="selectedTabs.length === renderTab.length" @change="toggleAll">全部</a-checkable-tag>
    <template v-for="tab in renderTab">
      <a-checkable-tag v-if="tab.dictionaryItemText" :key="tab.dictionaryItemCode" :checked="selectedTabs.indexOf(tab.dictionaryItemCode) > -1" @change="(checked) => handleChange(tab, checked)">
        {{ tab.dictionaryItemText }}
      </a-checkable-tag>
    </template>
    <div class="_list-search">搜索：<a-input-search placeholder="搜索" style="width: 150px" enter-button @search="onSearch" /></div>
  </div>
</template>
