<template>
	<div class="s-month-range-picker" :style="{ width: width }">
		<a-range-picker
...
export default {
	...
	props: {
		value: {
			...
			// 	endTime: moment().add(1, 'months').endOf('month').format('YYYY-MM-DD HH:mm:ss')
			// })
		},
		disabledDate: {
			type: Function
	...
	methods: {
		...
		emitDate() {
			let result = {
				startTime: this.selectedTime[0] ? this.selectedTime[0].startOf('month').format('YYYY-MM-DD 00:00:00') : '',
				endTime: this.selectedTime[1] ? this.selectedTime[1].endOf('month').format('YYYY-MM-DD HH:mm:ss') : ''
			}
