<template>
	<div class="s-minute-picker" :style="{ width: width }">
		<a-date-picker
...
export default {
	...
	props: {
		...
		showTime: {
			type: Object,
			default: () => ({ defaultValue: moment('00:00:00', 'HH:mm:ss'), format: 'HH:mm' })
		},
		isAllowClear: {
			type: Boolean,
			default: false
	...
	data() {
		return {
			selectedTime: moment().format('YYYY-MM-DD HH:mm:00')
		}
	},
	// 监听
	watch: {
	...
	methods: {
		...
		watchDate(newVal, oldVal) {
			...
			// 重置
			if (this.useDefaultTime && !newVal) {
				this.selectedTime = moment().format('YYYY-MM-DD HH:mm:00')
				this.emitDate()
				return
			}
		...
		emitDate() {
			const result = {
				startTime: this.selectedTime ? moment(this.selectedTime).format('YYYY-MM-DD HH:mm:00') : '',
				endTime: this.selectedTime ? moment(this.selectedTime).format('YYYY-MM-DD HH:mm:59') : ''
			}
