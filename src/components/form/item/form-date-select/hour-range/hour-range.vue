<template>
	<div class="s-hour-range" :style="{ width: width }">
		<a-range-picker
...
export default {
	...
	props: {
		value: {
			type: [Object, String]
			// default: () => ({
			// 	startTime: moment().startOf('hour').format('YYYY-MM-DD HH:00:00'),
			// 	endTime: moment().add(1, 'hours').endOf('hour').format('YYYY-MM-DD HH:59:59')
			// })
		},
		disabledDate: {
			type: Function
	...
	data() {
		return {
			selectedTime: [moment(), moment().add(1, 'hours')]
		}
	},
	watch: {
		value(newVal, oldVal) {
	...
	methods: {
		...
		initDate(newVal, oldVal) {
			...
			// 重置
			if (this.useDefaultTime && !newVal) {
				this.selectedTime = [moment(), moment().add(1, 'hours')]
				this.emitDate()
				return
			}
			//初始化的时候传进来是对象
		...
		emitDate() {
			let result = {
				startTime: this.selectedTime[0] ? this.selectedTime[0].format('YYYY-MM-DD HH:00:00') : '',
				endTime: this.selectedTime[1] ? this.selectedTime[1].format('YYYY-MM-DD HH:59:59') : ''
			}
