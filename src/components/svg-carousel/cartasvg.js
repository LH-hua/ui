...
export function cartaSvg(cfg) {
  ...
  var extend = function (dst, src) {
    ...
    for (var prop in src) if (src[prop] !== undefined) dst[prop] = src[prop]
    return dst
  }
  var attr = function (dst, src) {
    ...
    for (var prop in src) if (src[prop]) dst.setAttribute(prop, src[prop])
    return dst
  }
  ...
  root = append(cont, "svg", {
    ...
    height: cfg.height ? cfg.height : el.offsetWidth / 1.5,
    version: "1.1",
    xlmns: SVG_NS
  })
  ...
  extend(this, {
    ...
    paintBar: function () {
      ...
      var pts = []
      // plus round
      for (var i = -6;i <= cols + 6;i++) pts.push((mx = (w / 2) * Math.cos(i * anglestep)), (my = (-w / 2) * Math.sin(i * anglestep)))
      pts.push(-w / 5, -d / 2)
      pts.push(-d / 2, -d / 2)
      pts.push(-d / 2, -w / 5)
      pts.push(d / 2, -w / 5)
      ...
      pts.push(mx, my)
      // minus round
      for (var i = -6;i <= -6;i++) pts.push((-w / 2) * Math.cos(i * anglestep), h / 2 + (w / 2) * Math.sin(i * anglestep))
      pts.push(-w / 5, h / 2 - d / 2)
      pts.push(w / 5, h / 2 - d / 2)
      pts.push(w / 5, h / 2 + d / 2)
      pts.push(-w / 5, h / 2 + d / 2)
      pts.push(-w / 5, h / 2 - d / 2)
      for (var i = -6;i <= cols + 6;i++) pts.push((mx = (-w / 2) * Math.cos(i * anglestep)), (my = h / 2 + (w / 2) * Math.sin(i * anglestep)))
      // home round
      for (var i = 0;i <= cols;i++) pts.push((w / 6) * Math.cos(i * 2.0 * anglestep), h / 2 - h / 4 + (w / 6) * Math.sin(i * 2.0 * anglestep))
      pts.push(mx, my)
      var dx = tleft + w / 2,
        dy = ttop + h / 4,
    ...
    toPoints: function (coords, dotransform) {
      ...
      var pts = [coords[0] * this.m.delta + this.m.halfX, -coords[1] * this.m.delta + this.m.halfY]
      ...
      if ("Proj4js" in window) {
        ...
        if (!isNaN(destpt.x) && !isNaN(destpt.y)) {
          if (sourceproj.projName == "longlat") {
            return [(destpt.x / destproj.a) * Proj4js.common.R2D, (destpt.y / destproj.a) * Proj4js.common.R2D, !isNaN(destpt.z)]
          } else {
    ...
    rotateCoords: function (coords, angle, centerof) {
      ...
      if (r > 0) {
        ...
        coords = [cx - r * Math.cos(roll + a), cy + r * Math.sin(roll + a)]
      }
      return coords
    },
    ...
    mousedown: function (ev) {
      ...
      if ((this.m.mbar = this.chkBar(spts))) {
        // if bar
        return
      } else if (this.isTurnable()) {
        // proj.center for spherical turn
        ...
        // for drag
        this.m.mpts = [pts[0] / this.m.scale - this.m.offset[0], pts[1] / this.m.scale - this.m.offset[1]]
      }
    },
    mouseup: function (ev) {
      var spts = this.canvasXY(ev),
  ...
  extend(root, {
    ...
    touchstart: function (ev) {
      self.m.dotouch = true
      var touches = ev.changedTouches
      for (var i = 0;i < touches.length;i++) self.m.touches.push(touches[i])
      if (touches.length) self.mousedown(touches[0])
    },
    touchend: function (ev) {
      var touches = ev.changedTouches
      for (var i = 0;i < touches.length;i++) {
        for (var j = 0;j < self.m.touches.length;j++) {
          if (self.m.touches[j].identifier == touches[i].identifier) self.m.touches.splice(j, 1)
        }
      }
      if (!self.m.touches.length) self.mouseup(touches[touches.length - 1])
    },
    onmouseleave: function () {
      let mouseUpEvent = new Event('mouseup')
      root.dispatchEvent(mouseUpEvent)
    },
